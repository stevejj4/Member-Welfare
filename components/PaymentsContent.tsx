...
const PaymentsContent: React.FC<PaymentsContentProps> = ({ data }) => {
  const stats = [
    { label: 'Total Payments', value: 'KES 12,914,743', icon: <Repeat size={20} />, color: 'bg-[#00B5E2]' },
    { label: 'Successful', value: '12,914,743', icon: <CheckCircle size={20} />, color: 'bg-[#00B5E2]' },
    { label: 'Allocated', value: '84,973', icon: <FileCheck size={20} />, color: 'bg-[#00B5E2]' },
    { label: 'Un-Allocated', value: '9,547', icon: <FileX size={20} />, color: 'bg-[#00B5E2]' },
    { label: 'Unsuccessful', value: '0', icon: <XCircle size={20} />, color: 'bg-[#00B5E2]' },
  ];
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      <div className="mb-8">
        <h1 className="text-[18px] font-bold text-slate-700 uppercase tracking-tight mb-2">TRANSACTIONS</h1>
        <div className="flex items-center gap-2 text-[14px] text-slate-400">
          <span>Home</span>
          <span className="text-slate-300">/</span>
          <span className="font-medium text-slate-500">Transactions</span>
        </div>
      </div>
        ...
        {stats.map((card, i) => (
          <div key={i} className="bg-white p-6 rounded-lg border border-slate-100 shadow-sm flex items-center justify-between">
            <div className="space-y-3">
              ...
              <h3 className="text-[18px] font-extrabold text-slate-800">{card.value}</h3>
            </div>
            <div className={`w-10 h-10 ${card.color} rounded-full flex items-center justify-center text-white shadow-lg shadow-cyan-50`}>
              {card.icon}
            </div>
          </div>
        ))}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-100 p-8 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-6 mb-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input type="text" defaultValue="02/01/2026" className="w-full bg-white border border-slate-200 rounded-md px-4 py-2.5 text-slate-600 text-[13px] focus:outline-none focus:ring-1 focus:ring-cyan-500" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-[13px] font-bold text-slate-700">Transaction Code</label>
            <input type="text" placeholder="Enter Transaction Code" className="w-full border border-slate-200 rounded-md px-4 py-2.5 text-[13px] focus:ring-1 focus:ring-cyan-500 outline-none" />
          </div>
          <div className="space-y-2">
            <label className="text-[13px] font-bold text-slate-700">Account Number</label>
            <input type="text" placeholder="Enter Account Number" className="w-full border border-slate-200 rounded-md px-4 py-2.5 text-[13px] focus:ring-1 focus:ring-cyan-500 outline-none" />
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead className="bg-[#F1F5F9] border-b border-slate-200">
              <tr className="text-[11px] font-black text-[#475569] uppercase tracking-wider">
                ...
                <th className="py-4 px-6">PRINCIPAL NAME</th>
                <th className="py-4 px-6">ACCOUNT NUMBER</th>
                <th className="py-4 px-6">ID NUMBER</th>
                <th className="py-4 px-6">Reallocated Payment</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {(!data || data.length === 0) ? (
                <tr>
                  <td colSpan={14} className="text-center text-slate-500 py-10">No data available</td>
                </tr>
              ) : (
                data.map((row) => (
                  <tr key={row.id} className="text-[13px] text-slate-600 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-4 px-6 font-bold text-slate-800">{row.txId}</td>
                    <td className="py-4 px-6">{row.amount}</td>
                    <td className="py-4 px-6">{row.date}</td>
                    <td className="py-4 px-6">{row.principal}</td>
                    <td className="py-4 px-6">{row.account}</td>
                    <td className="py-4 px-6">{row.idNo}</td>
                    <td className="py-4 px-6">{row.reallocated}</td>
                    <td className="py-4 px-6">{row.group}</td>
                    <td className="py-4 px-6 font-medium text-[11px] text-slate-400">{row.method}</td>
                    <td className="py-4 px-6">
                      <span className="px-2.5 py-0.5 bg-[#E6F9F0] text-[#00B58D] text-[11px] font-bold rounded-md">
                        {row.status}
                      </span>
                    </td>
                    <td className="py-4 px-6">{row.allocated}</td>
                    <td className="py-4 px-6 text-center">
                      <button className="text-[#00B5E2] hover:bg-cyan-50 p-1.5 rounded-full"><Eye size={18} /></button>
                    </td>
                  </tr>
                ))
              )}

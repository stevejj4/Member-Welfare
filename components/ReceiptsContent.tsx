...
const ReceiptsContent: React.FC<ReceiptsContentProps> = ({ onViewReceipt, data }) => {
  const stats = [
    { label: 'Total Payments', value: '12,916,343', icon: <Repeat size={20} />, color: 'bg-[#00B5E2]' },
    { label: 'Successful Transactions', value: '12,916,343', icon: <CheckCircle size={20} />, color: 'bg-[#00B5E2]' },
    { label: 'Pending Transactions', value: '0', icon: <XCircle size={20} />, color: 'bg-[#00B5E2]' },
    { label: 'Unsuccessful Transactions', value: '0', icon: <XCircle size={20} />, color: 'bg-[#00B5E2]' },
  ];
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      <div className="mb-8">
        <h1 className="text-[18px] font-bold text-slate-700 uppercase tracking-tight mb-2">RECEIPTS</h1>
        <div className="flex items-center gap-2 text-[14px] text-slate-400">
          <span>Home</span>
          <span className="text-slate-300">/</span>
          <span className="font-medium text-slate-500">Receipts</span>
        </div>
      </div>
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead className="bg-[#F1F5F9] border-b border-slate-200">
              <tr className="text-[11px] font-black text-[#475569] uppercase tracking-wider">
                ...
                <th className="py-4 px-6">TRANSACTION NUMBER</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {(!data || data.length === 0) ? (
                <tr>
                  <td colSpan={8} className="text-center text-slate-500 py-10">No data available</td>
                </tr>
              ) : (
                data.map((row) => (
                  <tr key={row.id} className="text-[13px] text-slate-600 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-4 px-6 font-bold text-slate-800">{row.receiptNo}</td>
                    ...
                    <td className="py-4 px-6 font-medium text-slate-400 tracking-wider">{row.txNo}</td>
                    <td className="py-4 px-6 text-slate-500">{row.date}</td>
                    <td className="py-4 px-6">
                      <button 
                        onClick={() => onViewReceipt && onViewReceipt(row)}
